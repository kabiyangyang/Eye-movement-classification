...
def evaluate_test(model, X, y=None,
    ...
    for x_item, y_item in zip_equal(X, y):
        ...
        padding_size_x = [int(elem + padding_features) for elem in padding_size_x]
        # no @y_item-padding required

        if temporal_padding:
            padding_size_x = [int(elem + window_length / 2) for elem in padding_size_x]
        ...
        # Extract all valid windows in @padded_x, with given downsampling and size.
        # @res_X will have windows of size @window_length + 2*@padding_features
        window_length_x = int(window_length + 2 * padding_features)
        res_X += [padded_x[i:i + window_length_x, :] for i in
                  range(0, padded_x.shape[0] - window_length_x + 1, downsample)]
        if y_item is not None:
            # @res_Y will have windows of size @window_length, central to the ones in @res_X
    ...
    batch_size = model.get_config()['layers'][0]['config']['batch_input_shape'][0]  #added layers......
...
def run(args):
    ...
    for i, video_name in enumerate(all_video_names):
        ...
        file= open(MODEL_PATH + '/' + 'without_' + video_name+'.txt', mode='w+', encoding='UTF-8')
        ...
        for stat_name in ['FIX', 'SACC', 'SP', 'NOISE']:
            ...
            tmp_str = 'F1-{}'.format(stat_name) + ' ' + str(results['F1-{}'.format(stat_name)][-1])
            file.write(tmp_str)
        file.close()
...
def parse_args(dry_run=False):
    ...
    parser.add_argument('--num-conv', default=4, type=int,
    ...
    parser.add_argument('--num-blstm', default=2, type=int,
    ...
    if 'flow' in args.features:
        args.features.remove('flow')
        args.features += ['flow_speed', 'speed_dis', 'dir_dis']
    args.features = list(sorted(set(args.features)))

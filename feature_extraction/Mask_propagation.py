...
def parse_arguments():
    ...
    parser.add_argument('--flow-path', default='H:/of_data/',
                        help='The path that contains the flow data')
    
    parser.add_argument('--video-path', default='E:/movies-m2t/',
                        help='The path that contains the flow data')
    
    parser.add_argument('--mask-path', default='H:/gazeCom_mask/',
                        help='The path that contains the flow data')
...
def add_new_mask(mog_mask, propagated_nxt, res):
    tmp_res = mog_mask * propagated_nxt/255
...
def run(args):        
    ...
    VIDLIST =  ['bridge_2']#['holsten_gate','puppies', 'roundabout', 'sea', 'st_petri_gate', 'street', 'st_petri_mcdonalds']
    for v in VIDLIST:
        flow_path = args.flow_path + v + '/'
        
        vid = args.video_path + v + '.m2t'
        PATH = args.mask_path + v +  '/final_mask_arr.h5'#'E:/movies-m2t/st_petri_mcdonalds/seg/final_mask_arr.h5'
       # PATH2 = 'D:/Masks/mog_masks/MOG_' + v + '.h5'
        OUTPUTPATH = args.output_path + 'MOT_' + v + '.h5' 
        OUTPUTVIDEOPATH = args.outputvideo_path +'MOT_'+ v + '.avi'
